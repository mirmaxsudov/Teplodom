...
const Sort = () => {
    ...
    return (
        <section className="sort-section py-[30px] lg:px-[150px]">
            <div className="container mx-auto px-[50px] md:px-[20px]">
                {/* For Mobile & Sidebar */}
                <div className={`${isOpenSidebar ? "block" : "hidden"} md:hidden transition-all duration-500 fixed w-[100%] top-0 left-0 p-5 bg-[#fff] h-[100vh]`}>
                    <Link to={"/"}>
                        <div className="logo-wrapper flex gap-[14px] items-center justify-between">
                            <div className="flex items-center gap-[14px] my-10">
                                <img className="w-[40px] h-[40px] sm:w-[70px] sm:h-[70px]" src={Logo} alt="logo" />
                                <div>
                            ...
                            <div>
                                <button onClick={() => setIsOpenSidebar(!isOpenSidebar)} className="bg-[#F7F7F7] p-2 rounded-full transition-all duration-1000
                ...
                <div className="flex gap-5 md:hidden">
                    <button onClick={() => setIsOpenSidebar(!isOpenSidebar)}>
                        <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M4.375 9.625L7 7L4.375 4.375M10.5 7H23.625H10.5ZM4.375 14H23.625H4.375ZM4.375 21H23.625H4.375Z" stroke="black" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
                        </svg>
                    </button>
                ...
                <div className="hidden md:flex justify-between items-center">
                    {
                        sortItems.map(item => <SortItem item={item} key={uuid()} />)
                    }
                </div>
            </div>
        </section >
